...
double ILLutil_norm_sqr (double *v, int len)
{
   int  i;
   double sum = 0.0;

   for (i=0; i<len; i++) sum += v[i]*v[i];
   return sum;
}
...
const char *ILLutil_strchr (const char *s, int c)
{
    while (*s) {
        if (*s == c) return s;
        s++;
...
int ILLutil_index(const char* list[], const char* name)
{
    int i; 
...
int ILLutil_array_index(char* list[], int n, const char* name)
{
    int i; 
    for (i = 0;  i < n ; i++) {
