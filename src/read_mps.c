		      ...
		      qsline_reader *file, const char* fname)
{
    ...
    ILL_FAILtrue(file == NULL, "need file"); 
...
static char get_double(ILLread_mps_state *state, char peek, double *coef) {
    char ok = 0;
    int len, rval = 0;

    ILL_FAILfalse(state != NULL, "must have state"); 
    if (mps_skip_comment(state)) return 0;
...
int ILLmps_next_coef(ILLread_mps_state *state, double *coef) 
{
    int len = 0;
...
int ILLmps_next_bound(ILLread_mps_state *state, double *coef) 
{
    int len = 0,  sign = 1; 
    char c, *p; 
    ...
    if (!mps_skip_comment(state)) { 
        ...
        if (!strncasecmp (state->p+len, "INFINITY", 8) ) { 
	    len += 8; 
        } else { 
	    if (!strncasecmp (state->p+len, "INF", 3)) {
		len += 3;
	    }
        }
        if (len > 1) { 
...
static void mps_err(ILLread_mps_state *state, int isError, 
                  const char* format, va_list args)
{
	...
	ILL_FAILfalse_no_rval(format != NULL, "format != NULL"); 
	ILL_FAILfalse_no_rval(format[0] != '\0', "format[0] != '0'"); 
	ILL_FAILfalse_no_rval(state != NULL, "state != NULL"); 
	ILL_FAILfalse_no_rval(state->file != NULL, "state->file != NULL"); 
