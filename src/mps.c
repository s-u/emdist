...
static int read_mps_objnamesense(ILLmps_section sec, 
                                 ILLread_mps_state *state, rawlpdata *lp); 
...
static void mps_set_bound(rawlpdata *lp, ILLread_mps_state *state, 
			  int colind, const char* bndtype, double bnd);
...
static int read_mps_line_in_section(ILLread_mps_state *state, rawlpdata *lp)
{
	...
	return ILLmps_error(state, "Line is in no section.\n");
	    ...
	    switch (state->active) {
	    case ILL_MPS_ROWS:
		rval = add_row(state, lp); break;
	    case ILL_MPS_COLS:
		rval = add_col(state, lp); break;
	    case ILL_MPS_RHS:
		rval = add_rhs(state, lp); break;
	    case ILL_MPS_RANGES: 
		rval = add_ranges(state, lp); break;
	    case ILL_MPS_BOUNDS:
		rval = add_bounds(state, lp); break;
	    default: 
		ILL_REPRT("should never get here"); 
                ILL_CLEANUP;
	    }
...
static int add_row(ILLread_mps_state *state, rawlpdata *lp)
{
	...
	if (ILLmps_next_field(state) == 0) {
		...
		rval = ILLraw_add_row(lp, state->field, sense, 
				      0.0);  /* default rhs is 0.0 */
	    }
	} else {
...
static int mps_read_col_line(ILLread_mps_state *state, rawlpdata *lp)
{
	...
	if (hit) {
        ...
        if (ILLmps_next_coef(state, &coef) != 0) { 
	    return ILLmps_error(state, 
                             "Missing/Bad coefficient in COLUMNS record.\n");
	}
        rval = ILLraw_add_col_coef(lp, colind, rowind, coef);  
    }
CLEANUP:
...
static int is_marker_line(ILLread_mps_state *state) 
{
    ...
    const int slen = 8; /* strlen*"'MARKER'"); */
    while ((field = ILLutil_strchr(field, '\''))) {
	if (strncmp(field, "'MARKER'", slen) == 0) {
	    return 1; 
	} 
...
static int add_rhs (ILLread_mps_state * state, rawlpdata *lp)
{
	...
	for (more_fields = 1; more_fields; 
	     more_fields = !ILLmps_next_field(state)) { 
		...
		return ILLmps_error(state, 
                                 "Missing/Bad coefficient in RHS record.\n");
	    }
	    if (lp->rhsind[rowind]) {
...
static int add_bounds(ILLread_mps_state *state, rawlpdata *lp) 
{
	...
	if (strcmp(bounds_name, " ")) {
	    ...
	    if (ILLmps_next_field(state)) {
		return ILLmps_error(state, 
                                 "Missing column field in BOUNDS record.\n");
	    }
	}
			  ...
			  int colind, const char* bndtype, double bnd)
{
    ...
    if (!strcmp (bndtype, "LO")) {
        ...
        msg = ILLraw_set_lowerBound(lp, colind, -ILL_MAXDOUBLE); 
...
static int add_ranges (ILLread_mps_state *state, rawlpdata *lp)
{
	...
	if (strcmp(rangesname, " ")) { 
	    ...
	    if (ILLmps_next_field(state)) {
		return ILLmps_error(state, 
                                   "Missing row name in RANGES record.");
	    } 
	} 
	for (more_fields = 1; more_fields; 
	     more_fields = !ILLmps_next_field(state)) { 
	    ...
	    if (ILLmps_next_coef(state, &range)) { 
		return ILLmps_error(state, 
		...
		if (lp->rowsense[rowind] != 'N') {  
                    if (ILLraw_add_ranges_coef(lp, rowind, range)) 
			return 1; 
                } else {
...
static int mps_write_col(int i, int iorig, char *colname, ILLlpdata *lp, 
		  char** rownames, int intmode, char *objname);
...
{    
    ...
    for (i = 0; i < lp->nrows; i++) {
        if (lprows->rowcnt[i] == 0) {
            ILLdata_warn(collector, 
			 "Deleting empty row \"%s\" from constraints.", 
			 rownames[i]);
            continue;
        }
	switch (lp->sense[i]) {
    ...
    for (i = 0; i < lp->nrows; i++) {
        if ((lprows->rowcnt[i] != 0) && (lp->rhs[i] != 0.0)) { 
	    ILLprint_report (lp, " RHS    %s    %g\n", rownames[i], lp->rhs[i]);
	}
    } 
    ...
    if (lp->rangeval) {
        ...
        for (i = 0; i < lp->nrows; i++) {
            if ((lprows->rowcnt[i] != 0) && (lp->rangeval[i] != 0.0)) {
                ILLprint_report (lp, " RANGE    %s    %g\n", rownames[i],
				 lp->rangeval[i]); 
            }
        }
    }
	...
	for (ri = ri; ri < lp->nstruct; ri++) {
		...
		ILLprint_report (lp, " FX BOUND    %s    %g\n", colnames[ri], 
				 lp->lower[i] );
		continue;
	    }
		...
		if (lp->lower[i] == -ILL_MAXDOUBLE) {
		    ...
		    ILLprint_report (lp, " LO BOUND    %s    %g\n", colnames[ri],
				     lp->lower[i]);
		}
	    }
	    if (prtUpper) { 
		if (lp->upper[i] == ILL_MAXDOUBLE) {
		    ...
		    ILLprint_report (lp, " UP BOUND    %s    %g\n", colnames[ri],
				     lp->upper[i]);
		}
	    }
	}
