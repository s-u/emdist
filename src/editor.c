...
static void getCmd(ILLread_lp_state *state, int *cmd, int *subcmd) 
{
    ...
    *subcmd = -1; 
    if (hasSubCmd[*cmd] && 
	(ILLread_lp_state_next_field_on_line(state) == 0)) { 
...
void ILLeditor(QSdata* p) 
{
    ...
    while (ILLread_lp_state_next_field(state) == 0) {  
       ...
       switch (cmd) {
       case EXIT: ILL_BREAK_BODY; 

       case ROW: {
          switch (subcmd) {
          case ADD: add_row(p, lp, state); break;
             /* case NEW: rval = new_row(p, lp, state); break; */
          case DEL: del_row(p, lp, state); break;
          default: ILLeditor_help(); break;
          }
          break;
       }
       case COL: {
          switch (subcmd) {
          case ADD: add_col(p, lp, state); break;
          case DEL: del_col(p, lp, state); break;
          default: ILLeditor_help(); break;
          }
          break;
       }
       ...
       case PRTX: {
          if ((rval = ILLlib_print_x(stdout, p->lp, (ILLlp_cache *) NULL,
                                     (double *) NULL, 1))) {
       ...
       case NONE:  fprintf(stdout, "Unknown command: %s\n", state->field); 
       default: ILLeditor_help(); break;
       } 
       fflush(stdout); 
...
static int pull_info_from_p(QSdata *p, rawlpdata *lp) 
{
    ...
    ncols = qslp->nstruct; 
    /* add rows to lp */ 
    ILLraw_add_row(lp, qslp->objname, 'N', 0.0);
    for (i = 0; i < nrows; i++) {
	ILL_FAILfalse(qslp->rownames[i] != NULL, 
		      "should have no NULL names");
	ILLraw_add_row(lp, qslp->rownames[i], 
		       qslp->sense[i], qslp->rhs[i]); 
    }
...
static char* get_row_col_name(QSdata *p, rawlpdata *lp, 
                              ILLread_lp_state *state, int doRow) 
{
    int rval = 0; 
    int ind;
    ...
    if (rname == NULL) { 
	ILLlib_findName(p->qslp, 
			doRow /* forRow */, rname, id,  buf); 
...
static int fill_matrix(rawlpdata *lp, ILLread_lp_state *state, ILLmatrix *m, 
		       double *obj, int n)
{
    int i, cnt, rval = 0;
    colptr *cp; 
    ...
    m->matrows = lp->nrows; 
    m->matcols = 1; 
    ILL_SAFE_MALLOC(m->matval, lp->ncols, double); 
	...
	for (cp = lp->cols[i]; cp != NULL; cp = cp->next) {
	    ILL_FAILfalse(cp->this == n, 
			  "n should be the only row around"); 
	    if (cp->coef != 0.0) { 
		val += cp->coef;
		cnt++; 
	    } 
...
static void add_row(QSdata *p, rawlpdata *lp, ILLread_lp_state *state)
{
	...
	if (rval != 0) {
	    ...
	    QSadd_rows(p, 1, m.matcnt, m.matbeg, m.matind, m.matval, 
		       rhs, sense, (const char **) &name); 
	} 
    }
CLEANUP:
    ILLmatrix_free(&m); 
    if (name != NULL) { 
	if (rval != 0) ILLsymboltab_delete(&lp->rowtab, name); 
...
CLEANUP:
    ILLmatrix_free(m); 
    if (name[0] != NULL) { 
       if (rval != 0) ILLsymboltab_delete(&lp->rowtab, name[0]); 
    ...
    ILLraw_clear_matrix(lp); 
    if (transposed) transpose(lp);  
    ILL_IFFREE(name[0], char);
} 

#if 0
#ifndef JAVA_PORT 
static void new_row(QSdata *p, rawlpdata *lp, ILLread_lp_state *state)
{
    int rval = 0; 
...
static int del_row_or_col(QSdata *p, rawlpdata *lp, ILLread_lp_state *state,
                           int isRow)
{
    int i[1], rval = 0; 
