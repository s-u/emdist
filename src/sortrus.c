...
char *ILLutil_linked_radixsort (char *data, char *datanext, char *dataval,
        int valsize)
{
...
void ILLutil_str_perm_quicksort (int *perm, char**len, int n)
{
    ...
    for (;;) {
        do i++; while (i < n && (strcmp(len[perm[i]], t) < 0));
        do j--; while (strcmp(len[perm[j]], t) > 0);
        if (j < i) break;
        ILL_SWAP (perm[i], perm[j], temp);
    }
    ILL_SWAP (perm[0], perm[j], temp);
...
void ILLutil_rselect (int *arr, int l, int r, int m, double *coord,
        ILLrandstate *rstate)
{
...
#ifdef  TRY_CODE
static void perm_init(int *perm, int offset, int n)
{
    int i; 
    for (i = 0; i < n; i++) {
	perm[i + offset] = i; 
