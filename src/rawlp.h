...
extern colptr* ILLcolptralloc(ILLptrworld *p); 
...
extern void ILLinit_rawlpdata (rawlpdata *lp, qserror_collector *collector);
extern void ILLfree_rawlpdata (rawlpdata *lp);
extern void ILLraw_clear_matrix (rawlpdata *lp); 

extern const char* ILLraw_rowname (rawlpdata *lp, int i); 
extern const char* ILLraw_colname (rawlpdata *lp, int i); 

extern int  ILLraw_add_col(rawlpdata *lp, const char* name, int intmarker);
extern int  ILLraw_add_row(rawlpdata *lp, const char *name, 
                          char sense, double rhs); 

extern int ILLraw_add_col_coef(rawlpdata *lp, int colind, int rowind, 
			      double coef);

extern int ILLraw_init_ranges(rawlpdata *lp) ;
extern int ILLraw_init_rhs(rawlpdata *lp) ;

extern int ILLraw_add_ranges_coef(rawlpdata *lp, int rowind, double coef);


extern int  ILLraw_add_sos(rawlpdata *lp, int sos_type); 
                                                /* add empty set with type*/
extern int  ILLraw_add_sos_member(rawlpdata *lp, int colind); 
                                                /* add col to last set */
extern int  ILLraw_is_mem_other_sos(rawlpdata *lp, int colind); 

extern int  ILLraw_set_rhs_name(rawlpdata *lp, const char* name, int *skip); 
extern int  ILLraw_set_bounds_name(rawlpdata *lp, const char* name, int *skip); 
extern int  ILLraw_set_ranges_name(rawlpdata *lp, const char* name, int *skip); 
extern void ILLprint_rawlpdata (rawlpdata *lp);

extern char* ILLraw_unique_name(ILLsymboltab *tab, char* prefix, int i);
extern int  ILLraw_fill_in_rownames(rawlpdata *lp); 

extern int   ILLraw_init_bounds(rawlpdata *lp) ;

extern const char* 
             ILLraw_set_lowerBound(rawlpdata *lp, int i, double bnd);
extern const char* 
             ILLraw_set_upperBound(rawlpdata *lp, int i, double bnd);
extern const char*
             ILLraw_set_fixedBound(rawlpdata *lp, int i, double bnd);
extern const char*
             ILLraw_set_binaryBound(rawlpdata *lp, int i);
extern const char*
             ILLraw_set_unbound(rawlpdata *lp, int colind);
extern int   ILLraw_fill_in_bounds(rawlpdata *lp); 

extern int   ILLraw_first_nondefault_bound(ILLlpdata *lp); 
extern int   ILLraw_default_lower(ILLlpdata *lp, int i);
extern int   ILLraw_default_upper(ILLlpdata *lp, int i);

extern int  ILLrawlpdata_to_lpdata (rawlpdata *raw, ILLlpdata *lp);

extern int  ILLdata_error(qserror_collector *collector, 
			  const char* format, ...) ;
extern void ILLdata_warn(qserror_collector *collector, 
			 const char* format, ...) ;
