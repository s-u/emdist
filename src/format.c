		          ...
		          int lineNum, const char* theLine, int atPos)
{
	...
	ILL_FAILtrue(mode >= QS_INPUT_NERROR || mode < 0, "0<= mode <=QS_INPUT_NERROR");
    error->type = mode; 
    len = strlen(desc); 
...
void
ILLformat_error_print(FILE *out, qsformat_error *error)
{ 
    int at = error->at;
    int tp = error->type;
...
qserror_memory* 
ILLerror_memory_create(char takeErrorLines)
{
	int rval = 0, i; 
