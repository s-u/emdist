...
static int read_objective(ILLread_lp_state *state, rawlpdata *lp); 
static int read_constraints(ILLread_lp_state *state, rawlpdata *lp, 
			    int allowNewCols);
static int read_colname(ILLread_lp_state *state, ILLsymboltab *coltab, 
                        char mustHave);
static int read_integer(ILLread_lp_state *state, rawlpdata *lp);
static int read_bounds(ILLread_lp_state *state, rawlpdata *lp);
static int add_var(rawlpdata *lp, ILLread_lp_state *state,
		   double coef, int row, int allowNew);

/*------------------------------------------------------------------------ 
 * ILLwrite_lp and support routines
 */
static int fix_names(qserror_collector *collector,
		     char **names, int nnames, const char* extra, char prefix,
		     char ***newnames);
static void write_objective(ILLlpdata *lp, 
                            const char *objname, char **colnames) ;
static int write_row(ILLlpdata * lp, ILLlp_rows *lprows, int i, 
                     char** rownames, char** colnames, 
                     int* colInRow, double* colCoef);
...
int ILLwrite_lp (ILLlpdata *lp, qserror_collector *collector)
{
    ...
    if (lp->sos.matcols > 0) {
	rval += ILLdata_error(collector, "Can't express SOS information in LP format.");
    }
    ...
    ILL_SAFE_MALLOC(colCoef, lp->nstruct, double); 
    ...
    for (i = 0; i < lp->nrows; i++) {
        if (lprows->rowcnt[i] == 0) {
            ILLdata_warn(collector, 
			     "Not printing  empty row \"%s\".", rownames[i]);
            continue;
        }
        rval += write_row(lp, lprows, i, rownames, colnames, 
                          colInRow, colCoef);  
    }
...
static void write_the_expr(ILLlpdata *lp, ILLwrite_lp_state *line, 
		           char *rowname, 
			   ILLlp_rows *lprows, int row, 
			   char** colnames, 
			   int* colInRow, double* colCoef, int ncols)
{
                     ...
                     int* colInRow, double* colCoef)
{
	...
	ILL_FAILtrue(!lp->rangeval, "RANGE constraints without values\n");
        ...
        ILLwrite_lp_state_append_number(line, lp->rhs[i]); 
        ILLwrite_lp_state_append(line, ", "); 
        ILLwrite_lp_state_append_number(line, lp->rhs[i] + lp->rangeval[i]); 
        ILLwrite_lp_state_append(line, ")"); 
        ILLprint_report(lp, "%s\n", line->buf); 
 ...
 */
static int fix_names(qserror_collector *collector,
		     char **names, int nnames, const char* extra, char pref,
		     char ***newnames)
{
 ...
 */
    ...
    if (rval == 0) rval = read_problem_name(state, lp);  
    if (rval == 0) rval = read_minmax(state, lp);              
    if (rval == 0) rval = read_objective(state, lp); 
    if (rval == 0) rval = read_constraints(state, lp, 1);
...
int ILLread_constraint_expr(ILLread_lp_state *state, rawlpdata *lp, 
			        int rowind, int allowNew)
{
	...
	haveCoef = ILLread_lp_state_possible_coef(state, &coef, 1.0); 
	if (ILLread_lp_state_next_var(state) == 0) { 
            double v = sign * coef; 
	    rval = add_var(lp, state, v, rowind, allowNew); 
	    ILL_CLEANUP_IF(rval);
	} else { 
	    if (haveCoef == 0) {
		return ILLlp_error(state, 
				   "Coefficient without variable.\n");
	    }  else {
		break;
	    }
	} 
...
static int read_objective(ILLread_lp_state *state, rawlpdata *lp) 
{
	...
	if (name == NULL) { 
        ...
        rval = ILLraw_add_row(lp, objname, 'N', 0.0); 
                            ...
                            rawlpdata *lp, int allowNewCols)
{
    ...
    rowind = lp->nrows; 
    rval = rval || ILLraw_add_row(lp, rowname, 'N', 0.0); 

    rval = rval || ILLread_constraint_expr(state, lp, rowind,
					   allowNewCols);
	...
	ILL_IFTRACE("SENSE \"%s\": %c %f\n", 
		    ILLraw_rowname(lp, rowind), sense, d); 
    }
CLEANUP:
...
static int read_integer(ILLread_lp_state *state, rawlpdata *lp) 
{
    ...
    while ((rval = read_colname(state, coltab, 0)) == 0) { 
        ILL_FAILtrue(state->column_index == ILL_SYM_NOINDEX, 
		     "Programming error");  
...
static int read_bounds(ILLread_lp_state *state, rawlpdata *lp) 
{
	...
	if (ILLread_lp_state_possible_bound_value(state)) {
	    ...
	    msg = ILLraw_set_lowerBound(lp, colind, bound);  
		...
		bound = state->bound_val; 
		if (sense == 'E') {  
		    msg = ILLraw_set_fixedBound(lp, colind, bound); 
		} else {
		    msg = ILLraw_set_upperBound(lp, colind, bound);  
		}
		ILLlp_warn(state, msg); 
		haveBound = 1;
	    } else {
        ...
        ILL_IFTRACE("BOUNDS: %f <= %s <= %f", lp->lower[colind], 
                    ILLraw_colname(lp, colind), lp->upper[colind]); 
    }
    ILLread_lp_state_next_field(state); 
CLEANUP:
		   ...
		   double coef, int row, int allowNew)
{
    ...
    ILL_IFTRACE("add_var: \"%s\" coef=%f row=%s\n", 
		var, coef, ILLraw_rowname(lp, row)); 
